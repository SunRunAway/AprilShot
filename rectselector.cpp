...
void RectSelector::initSelectedMenu()
{
    cancelAction = new QAction(tr("Retry"),this);
    quitAction = new QAction(tr("Quit"),this);
    contextMenu = new QMenu(this);

    connect(cancelAction,SIGNAL(triggered()),this,SLOT(cancelSelectedRect()));
    connect(quitAction,SIGNAL(triggered()),this,SLOT(quitSelector()));
}

void RectSelector::loadBackgroundPixmap(const QPixmap &bgPixmap)
{
    int width,height;
...
void RectSelector::loadBackgroundPixmap(const QPixmap &bgPixmap, int x, int y, int width, int height)
{
    loadPixmap = bgPixmap;
    screenx = x;
    screeny = y;
...
QPixmap RectSelector::getFullScreenPixmap()
{
    initRectSelector();
...
void RectSelector::keyPressEvent(QKeyEvent *event)
{
...
void RectSelector::mouseReleaseEvent(QMouseEvent *event)
{
...
QRect RectSelector::getRect(const QPoint &beginPoint, const QPoint &endPoint)
{
...
void RectSelector::initRectSelector()
{
...
bool RectSelector::isInSelectedRect(const QPoint &point)
{
    int x,y;
    QRect selectedRect;
...
QRect RectSelector::getSelectedRect()
{
...
void RectSelector::updateBeginEndPointValue(const QRect &rect)
{
...
void RectSelector::checkMoveEndPoint()
{
    int x,y;
...
void RectSelector::updateMouseShape(const QPoint &point)
{
    switch(currentShotState){
    case initShot:
        break;
...
void RectSelector::updateMoveControlMouseShape(controlPointEnum controlValue){
    switch(controlValue){
    case moveControl1:
    case moveControl5:
...
RectSelector::controlPointEnum RectSelector::getMoveControlState(const QPoint &point)
{
    RectSelector::controlPointEnum result = moveControl0;
    if(currentShotState == initShot || currentShotState == beginShot)  {
        result = moveControl0;
    }
...
QRect RectSelector::getMoveAllSelectedRect(void)
{
    QRect result;
...
QRect RectSelector::getMoveControlSelectedRect(void)
{
    int x,y,w,h;
